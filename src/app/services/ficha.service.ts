...
export class FichaService {
  ...
  public cadastrarNovaFicha(ficha: FichaPaciente): Observable<FichaPaciente> {
    let header = this.tokenService.getTokenHeader();
    return this.http.post<FichaPaciente>(
      environment.apiURL + '/fichas',
      ficha,
      { headers: header }
    );
  }
  ...
  public atualizarFicha(ficha: FichaPaciente): Observable<FichaPaciente> {
    let header = this.tokenService.getTokenHeader();
    return this.http.put<FichaPaciente>(
      environment.apiURL + '/fichas/' + ficha.idFicha,
      ficha,
      { headers: header }
    );
  }
  ...
  public buscarFichaPeloId(id: string): Observable<FichaPaciente> {
    let header = this.tokenService.getTokenHeader();
    return this.http.get<FichaPaciente>(environment.apiURL + '/fichas/' + id, {
      headers: header,
    });
  }
}
